...
-- Full-screen difficulty picker shown before the first level starts.
-- Fires SetDifficulty to the server, then hides itself.
...
-- Subtle scanline texture via repeated thin frames
for i = 0, 40 do
...
local startX = math.floor((1920 - totalW) / 2)  -- approximate center offset
...
local function makeCard(diff, index)
        ...
        for _, child in ipairs(bg:GetDescendants()) do
            if child:IsA("GuiObject") then
                TweenService:Create(child, TweenInfo.new(0.3),
                    child:IsA("TextLabel") and { TextTransparency = 1 } or { BackgroundTransparency = 1 }
                ):Play()
            end
        end
        task.wait(0.45)
    ...
    card.Position = UDim2.new(card.Position.X.Scale, card.Position.X.Offset,
        card.Position.Y.Scale, card.Position.Y.Offset + 40)
